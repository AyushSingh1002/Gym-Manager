...
export function Sidebar() {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed top-0 left-0 z-50 h-full w-64 bg-canvas border-r border-hairline flex flex-col transition-transform duration-300 lg:translate-x-0 lg:static lg:z-auto",
      ...
      >
        <div className="flex items-center justify-between h-14 px-5 border-b border-hairline flex-shrink-0">
          ...
          <button
            onClick={() => setSidebarOpen(false)}
            className="p-1 rounded-[var(--radius-sm)] text-ink-tertiary hover:text-ink hover:bg-surface-1 lg:hidden"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
